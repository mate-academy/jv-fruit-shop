...
public class FruitShopMain {
    ...
    public static void main(String[] args) {
        ...
        TransactionsEvaluateService evaluateService = new TransactionsEvaluateServiceImpl();
        GridWriteService gridWriteService = new CsvGridWriteService(new File(REPORT_FILE_PATH));
        // Processing transactions
        List<FruitTransaction> fruitTransactions = parser.parse(gridReadService.getRows());
        evaluateService.evaluate(fruitTransactions);
        // Report creating
        ValueOfFruit valueOfFruit = new RowValueOfFruit();
        gridWriteService.writeLines(valueOfFruit.valueOf(Storage.getFruits()),
