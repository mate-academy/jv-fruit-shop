...
public class Main {
    public static void main(String[] args) {
        ...
        ReportDataStorage reportDataStorage = new ReportDataStoragePerMapImpl(new HashMap<>());
        for (String s : input) {
            String[] separate = s.split(",");
            operationStrategy.getOperationHandler(separate[0])
                    .provideOperation(reportDataStorage, separate[1], Integer.parseInt(separate[2]));
        }
