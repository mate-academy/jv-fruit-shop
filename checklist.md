...
Make all services independent and call them in the right order in `main()` method step by step (the result of previous core.basesyntax.db.service method should be the input for next one)
...
- `core.basesyntax.db.service` for holding services, like Writer, Reader, Parser and so on
- `core.basesyntax.db.service.impl` for holding implementations of services
- `core.basesyntax.db.strategy` for holding handlers for different operations (you are expected to apply Strategy pattern)
...
In the `main()` method you can pass the core.basesyntax.db.strategy map into the core.basesyntax.db.service that chooses the correct core.basesyntax.db.strategy for each operation.

- Example:  
    ```java
    public static void main(String[] args){
        // create and fill the core.basesyntax.db.strategy map
