# Fruit shop
Let's imagine that we have a fruit store. Every day in the store there are a number of operation, 
...
There are four operation at the store:
```text
...
Let's check in details all types of operation:
...
NOTE: if while processing transactions you detect some business logic mistakes (e.g not enough fruits to buy) throw an exception from core.basesyntax.service or handlers.
