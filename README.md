...
......
Структура вашого проекту повинна складатися з таких пакетів:
- `db` для зберігання пам'яті
- `model` для утримування таких моделей, як Fruit (за потреби)
- `service` для зберігання таких служб, як Writer, Reader, Parser і так далі
- `service.impl` для зберігання реалізацій сервісів
- `strategy` для зберігання обробників для різних операцій (ви повинні застосувати шаблон стратегії)
   dao  - Забезпечення звʼязку
  б - залишок, залишки плодів на початок робочого дня
s - поставка, означає, що ви отримуєте нові фрукти від постачальників
p - покупка, означає, що хтось купив фрукти
r - повернення, означає, що той, хто купив фрукти, повертає їх назад

одним англійським словом